...
public class ConfirmGuiProvider implements InventoryProvider {
    ...
    public void init(Player player, InventoryContents contents) {
        contents.set(new SlotPos(0, 2), ClickableItem.of(ItemBuilder.build(Material.GREEN_WOOL, "§a§lCONFIRM"), e -> {
            confirmCallback.onChoose(true);
            player.closeInventory();
        }));

        contents.set(new SlotPos(0, 5), ClickableItem.of(ItemBuilder.build(Material.RED_WOOL, "§a§lREFUSE"), e -> {
